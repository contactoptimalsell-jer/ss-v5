...
export default async function handler(
  ...
  if (!rateLimitCheck.canSend) {
    console.log(`❌ [send-pdf] Rate limit exceeded, blocking silently`);
  ...
  console.log(`🔒 [send-pdf] Attempting to lock email: ${email}`);
  const lockResult = await tryLockEmail(email);
  if (!lockResult.success) {
    console.log(`❌ [send-pdf] Email already locked, blocking silently`);
    // Bloquer silencieusement sans message d'erreur
    return res.status(200).json({ 
      success: true,
      message: 'PDF envoyé avec succès !' 
  ...
  console.log(`✅ [send-pdf] Email locked successfully, proceeding with PDF generation`);
